...
import static zlc.season.rxdownload2.db.Db.RecordTable.COLUMN_TIME;
import static zlc.season.rxdownload2.db.Db.RecordTable.COLUMN_TITLE;
...
import static zlc.season.rxdownload2.db.Db.RecordTable.COLUMN_VERSION;
...
import static zlc.season.rxdownload2.db.Db.RecordTable.readMissionId;
...
public class DataBaseHelper {
     ...
     *
     * @param url url duplicate column name: extra1 (code 1): , while compiling: ALTER TABLE download_record ADD extra1 TEXT
        ...
        try {
            cursor = getReadableDatabase().query(TABLE_NAME,
                    new String[]{COLUMN_ID, COLUMN_URL, COLUMN_SAVE_NAME, COLUMN_SAVE_PATH,
                            ...
                            COLUMN_EXTRA5, COLUMN_VERSION, COLUMN_TITLE, COLUMN_TIME, COLUMN_DOWNLOAD_FLAG, COLUMN_DATE, COLUMN_MISSION_ID},
                    COLUMN_URL + "=?", new String[]{url}, null, null, null);
            cursor.moveToFirst();
     ...
     * @return String
     */
    public List<String> readMissionIdAll() {
        Cursor cursor = null;
        try {
            cursor = getReadableDatabase().query(TABLE_NAME,
                    new String[]{COLUMN_MISSION_ID}, null, null, null, null, null);
            List<String> result = new ArrayList<>();
            cursor.moveToFirst();
            if (cursor.getCount() > 0) {
                do {
                    if (!result.contains(readMissionId(cursor)))
                        result.add(readMissionId(cursor));
                } while (cursor.moveToNext());
            }
            return result;
        } finally {
