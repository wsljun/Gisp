...
public class StepsView extends LinearLayout implements StepsViewIndicator.OnDrawListener {
    ...
    public FrameLayout getmLabelsLayout() {
        return mLabelsLayout;
    }
    ...
    public void drawView() {
        ...
        if (mCompletedPosition < 0 || mCompletedPosition > mLabels.length - 1) {
            throw new IndexOutOfBoundsException(String.format("Index : %MyScholarEntity, Size : %MyScholarEntity", mCompletedPosition, mLabels.length));
        }
    ...
    private void drawLabels() {
        ...
        if (mLabels != null) {
            for (int i = 0; i < mLabels.length; i++) {
                ...
                textView.setX(indicatorPosition.get(i)*1.1f);
